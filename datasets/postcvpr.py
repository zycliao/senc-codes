...
def create(mcfg: Config):
    ...
    if mcfg.single_sequence_file is not None:
        ...
        if hasattr(mcfg, 'betas_id'):
            datasplit['betas_id'] = [mcfg.betas_id]
    else:
...
class GarmentBuilder:
    ...
    def add_coarse(self, sample: HeteroData, garment_name: str, is_training=True) -> HeteroData:
        """
    ...
    def build(self, sample: HeteroData, sequence_dict: dict, idx: int, garment_name: str, is_training=True) -> HeteroData:
        """
        ...
        sample = self.add_coarse(sample, garment_name, is_training=is_training)
...
class Loader:
    ...
    def load_sample(self, fname: str, idx: int, garment_name: str, betas_id: int, is_training=True) -> HeteroData:
        """
        ...
        sample = HeteroData()
        sample = self.garment_builder.build(sample, sequence, idx, garment_name, is_training=is_training)
...
class Dataset:
    ...
    def __getitem__(self, item: int) -> HeteroData:
        ...
        sample = self.loader.load_sample(fname, idx, garment_name, betas_id=betas_id, is_training=not self.wholeseq)
        sample['sequence_name'] = fname
        sample['garment_name'] = garment_name
        # setattr(sample['cloth'], 'sequence_name', fname)
        # setattr(sample['cloth'], 'garment_name', garment_name)

        return sample

    def __len__(self) -> int:
        if self._len < 1000000:
            return 1000000
        else:
            return self._len
