...
def load_params(config_name: str=None, config_dir: str=None, add_self=False):
    """
    ...
    if config_name is None:
        ...
        if hasattr(conf_cli, 'config'):
            config_name = conf_cli.config
        elif hasattr(conf_cli, 'resume'):
            config_name = conf_cli.resume.split('_202')[0]
        elif hasattr(conf_cli, 'exp'):
            config_name = conf_cli.exp.split('_202')[0]
        else:
            raise ValueError('Please specify config name with --config or --resume')
    ...
    if add_self:
        if not hasattr(conf.criterions, 'postcvpr.mataug.self_intersection_energy'):
            conf.criterions['postcvpr.mataug.self_intersection_energy'] = DictConfig({'weight': 100, 'norm': 'l1'})
        next(iter(conf.runner.values())).self_col = True
