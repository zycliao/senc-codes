...
from utils_self_inter.mesh import faces_to_edges_and_adjacency, compute_boundary_paths, get_vertex_edge_dict_and_face_adj_dict
import einops
import numpy as np
import torch
...
def check_order(nv, v, v_idx, faces):
    for face in faces:
        if (face[0] == nv and face[1] == v) or (face[1] == nv and face[2] == v) or (face[2] == nv and face[0] == v):
            faces = np.vstack((faces, np.array([v, nv, v_idx])))
            return faces
        elif (face[0] == v and face[1] == nv) or (face[1] == v and face[2] == nv) or (face[2] == v and face[0] == nv):
            faces = np.vstack((faces, np.array([nv, v, v_idx])))
            return faces

def get_closed_garment(boundary_paths, faces):
    for num_path, boundary in enumerate(boundary_paths):
        v_idx = np.max(faces) + 1
        for i, v in enumerate(boundary):
            nv = boundary[(i + 1) % len(boundary)]
            faces = check_order(nv, v, v_idx, faces)
    return faces
...
def face_normals(vertices, faces, normalized=True):
    ...
    normals = torch.cross(v01, v12)
    if normalized:
        normals = normals / (
            torch.norm(normals, dim=-1, keepdim=True) + torch.tensor(1e-8).cuda()
        )
...
class Cloth:
    ...
    def make_connectivity_old(self, f):
        f_connectivity, f_connectivity_edges = get_face_connectivity_combined_old(f)
        return f_connectivity, f_connectivity_edges

    def make_connectivity(self, f):
        f_connectivity, f_connectivity_edges, f_adj_dict, boundary_paths, closed_faces = get_face_connectivity_combined(f)
        return f_connectivity, f_connectivity_edges, f_adj_dict, boundary_paths, closed_faces
    ...
    def make_continuum(self, v, f):
        device = v.device
        dtype = v.dtype
        normals = face_normals(v, f)
        normals = normals.detach().cpu().numpy()
        angle = np.arccos(normals[:, 2])
        axis = np.stack(
            [
                normals[:, 1],
                -normals[:, 0],
                np.zeros((normals.shape[0],), np.float32),
            ],
            axis=-1,
        )
        ...
        triangles = v[f].detach().cpu().numpy()
        triangles = np.einsum("abc,adc->abd", triangles, rotations)[..., :2]
        uv_matrices = np.stack(
            [triangles[:, 1] - triangles[:, 0], triangles[:, 2] - triangles[:, 0]],
            axis=-1,
        )
        uv_matrices = np.linalg.inv(uv_matrices)
        return torch.from_numpy(uv_matrices).to(device).to(dtype)
    ...
    def set_batch(self, batch, overwrite_pos=False):
        ...
        for i in range(B):
            ...
            if garment_name in self.cache:
                ...
                f_adj_dict = self.cache[garment_name]['f_adj_dict']
                closed_faces = self.cache[garment_name]['closed_faces']
                boundary_paths = self.cache[garment_name]['boundary_paths']
                ...
                if not self.always_overwrite_mass:
                    v_mass = self.make_v_mass(v, f, device)
                f_area = self.make_f_area(v, f, device)
                f_connectivity, f_connectivity_edges, f_adj_dict, boundary_paths, closed_faces = self.make_connectivity(f)
                Dm_inv = self.make_Dm_inv(v, f)
                ...
                self.cache[garment_name]['f_adj_dict'] = f_adj_dict
                self.cache[garment_name]['closed_faces'] = closed_faces
                self.cache[garment_name]['boundary_paths'] = boundary_paths
            ...
            example['cloth'].f_adj_dict = f_adj_dict
            example['cloth'].closed_faces = closed_faces
            example['cloth'].boundary_paths = boundary_paths
...
class ClothMatAug(Cloth):
    def __init__(self, material, always_overwrite_mass=False, self_col=False):
    ...
    def set_batch(self, batch, overwrite_pos=False):
        ...
        for i in range(B):
            example = batch.get_example(i)
            example['cloth'].energy_type = batch['cloth'].energy_type[i]
            garment_name = example.garment_name

            # example['cloth'].garment_name = batch['cloth'].garment_name[i]
            # example['cloth'].sequence_name = batch['cloth'].sequence_name[i]
            #
            # garment_name = example['cloth'].garment_name
            ...
            if garment_name in self.cache:
                ...
                if not overwrite_pos:
                    ...
                    f_area = self.cache[garment_name]['f_area']
                    Dm_inv = self.cache[garment_name]['Dm_inv']
                    uv_matrices = self.cache[garment_name]['uv_matrices']
                else:
                    if not self.always_overwrite_mass:
                        v_mass = self.make_v_mass(v, f, density, device)
                    f_area = self.make_f_area(v, f, device)
                    Dm_inv = self.make_Dm_inv(v, f)
                    uv_matrices = self.make_continuum(v, f)
            else:
                if not self.always_overwrite_mass:
                    v_mass = self.make_v_mass(v, f, density, device)
                f_area = self.make_f_area(v, f, device)

                if not self.self_col:
                    f_connectivity, f_connectivity_edges = self.make_connectivity_old(f)
                else:
                    # for self collision
                    f_connectivity, f_connectivity_edges, f_adj_dict, boundary_paths, closed_faces = self.make_connectivity(f)
                    num_f_adj = np.max(list(f_adj_dict.keys())) + 1
                    max_adj_num = np.max([len(k) for k in f_adj_dict.values()])
                    f_adj_list = torch.ones((num_f_adj, max_adj_num), dtype=torch.long) * -1
                    for k, vv in f_adj_dict.items():
                        adj_num = len(vv)
                        f_adj_list[k, :adj_num] = torch.tensor(vv, dtype=torch.long)
                    f_adj_dict = f_adj_list.numpy()
                    boundary_paths = [b.detach() for b in boundary_paths]
                ...
                self.cache[garment_name]['f_connectivity_edges'] = f_connectivity_edges
                self.cache[garment_name]['Dm_inv'] = Dm_inv
                self.cache[garment_name]['uv_matrices'] = uv_matrices
            ...
            example['cloth'].f_connectivity_edges = f_connectivity_edges
            example['cloth'].Dm_inv = Dm_inv
            example['cloth'].uv_matrices = uv_matrices
        ...
        new_batch = Batch.from_data_list(new_examples_list)
        # for k, v in batch._global_store.items():
        #     if k not in new_batch._global_store:
        #         new_batch._global_store[k] = v
        end = time.time()
        return new_batch
...
def get_face_connectivity_combined_old(faces):
    """
...
def get_face_connectivity_combined(faces):
    ...
    adjacency_edge_idx,
    boundary_edges_dict,
    face_adjacency_dict )= faces_to_edges_and_adjacency(faces)
    boundary_paths = compute_boundary_paths(boundary_edges_dict)
    closed_faces = get_closed_garment(boundary_paths, faces)

    f_adj_dict = dict()
    _, f_adj_dict = get_vertex_edge_dict_and_face_adj_dict(closed_faces)

    adjacent_faces = torch.LongTensor(adjacent_faces).to(device)
    adjacent_face_edges = torch.LongTensor(adjacent_face_edges).to(device)
    boundary_paths = [(torch.tensor(l)).to(device) for l in boundary_paths]

    return adjacent_faces, adjacent_face_edges, f_adj_dict, boundary_paths, closed_faces
