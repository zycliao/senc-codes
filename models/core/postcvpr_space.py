...
class GraphNetBlock(BaseBlock):
    ...
    def aggregate_node_features(self, sample, updated_edge_features):
        # for each node, sum  edge features of its incident edges
...
class EncodeProcessDecode(nn.Module):
    def __init__(self, mcfg):
        ...
        edgeset_encoders['space'] = self._make_mlp(self.n_edgefeatures_mesh, self._latent_size) # for self collision
        ...
        self.edge_key_transform["s"] = "space_cloth"
        ...
        self.edge_sets_full = make_edgesets_dict(self._n_coarse_levels, selfcoll=True)
    ...
    def _encode_edges(self, sample):
        ...
        # for self collision
        mesh_edge_features = sample['cloth', 'space_edge', 'cloth'].features
        mesh_edge_latents = self.edgeset_encoders['space'](mesh_edge_features)
        sample['cloth', 'space_edge', 'cloth'].features = mesh_edge_latents
