...
class Config:
    ...
    n_nodefeatures: int = 24            # number of dimensions in node feature vectors (don't chenge), originally 24, need to check this
    n_edgefeatures_mesh: int = 9       # number of dimensions in edge feature vectors (don't chenge), originally 12
    n_edgefeatures_coarse: int = 9     # number of dimensions in edge feature vectors for long-range edges (don't chenge), originally 12
...
class Model(nn.Module):
    def __init__(self, learned_model, mcfg: Config):
        ...
        self._node_normalizer = normalization.Normalizer(
            size=mcfg.n_nodefeatures)
        self._mesh_edge_normalizer = normalization.Normalizer(
            size=mcfg.n_edgefeatures_mesh)
    ...
    def create_mesh_edge_set(self, sample, is_training, edge_label, normalizer):
        ...
        edge_features_final = edge_features_normalized
    ...
    def add_node_features(self, sample):
        ...
        for k in ['cloth', 'obstacle']:
            ...
            #      lame_lambda], dim=-1)
            node_features = torch.cat(
                [velocity, explicit_acc, vertex_type_emb, vertex_level_emb, normals, timestep, v_mass], dim=-1)
    ...
    def normalize_node_features(self, sample, is_training):
        ...
        all_features_to_norm = all_features
        ...
        all_features_normalized_final = all_features_normalized
