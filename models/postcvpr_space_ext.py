...
class Config:
    ...
    n_nodefeatures: int = 27            # number of dimensions in node feature vectors (don't chenge)
...
class Model(nn.Module):
    def __init__(self, learned_model, mcfg: Config):
        ...
        self._space_edge_normalizer = normalization.Normalizer(
            size=mcfg.n_edgefeatures_mesh - 3)
    ...
    def prepare_inputs(self, sample, is_training):
        ...
        # for self collision
        sample = self.create_mesh_edge_set(sample, is_training, 'space_edge', self._space_edge_normalizer)
